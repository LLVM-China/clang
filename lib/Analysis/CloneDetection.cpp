...
void AutoGeneratedCloneConstraint::constrain(
    std::vector<CloneDetector::CloneGroup> &Result) {
  std::vector<unsigned> IndexesToRemove;
  unsigned i = 0;

  for (CloneDetector::CloneGroup &Group : Result) {
    if (Group.empty()) {
      i++;
      continue;
    }
